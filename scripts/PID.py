...
class Poruke:
    ...
    def reference_fun(self, data):
        ...
        #print "yyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy", data.y
...
if __name__ == '__main__':
    PID_x = PID(K_pf_num=0.7148, K_pf_den=0.2852, Kr=17.0, Ti=35.0, Td=0.4, v=10.0, Integrator_max=100.0, Integrator_min=-100.0, T_dis =0.084, Kr_i=0.0, Kr_d=0.010)

    PID_y = PID(K_pf_num=0.7148, K_pf_den=0.2852, Kr=12.0, Ti=35.0, Td=0.4, v=10.0, Integrator_max=100.0, Integrator_min=-100.0, T_dis = 0.084, Kr_i = 0.0, Kr_d= 0.010)

    PID_z = PID(K_pf_num=0.7198, K_pf_den=0.2802, Kr=2.50, Ti=30.0, Td=0.4, v=20.0, Integrator_max=50.0, Integrator_min=0.0, T_dis = 0.2439, Kr_i = 0.0008, Kr_d= 0.10
    )
    ...
    PID_yaw = PID(K_pf_num=0.7123, K_pf_den=0.2877, Kr=10.0, Ti=20.0, Td=0.1, v=10.0, Integrator_max=5.0, Integrator_min=0.0, T_dis = 0.084, Kr_i = 0.0082, Kr_d= 11.904762)
    ...
    while not rospy.is_shutdown():
        ...
        if signal_z_main > 50.0:
            signal_z_main = 50.0
        elif signal_z_main < 0.0:
            signal_z_main = 0.0        
        signal_z_main =(0.22/50.0)*signal_z_main+0.78 #signal_z_main * (0.8/100.0) +50.0*(0.8/100.0)- 0.4 +0.6  #signal_z_main*(0.3/100) +50*(0.3/100)-0.15 + 0.85 

        if signal_x_main > 100.0:
            signal_x_main = 100.0
        elif signal_x_main < -100.0:
            signal_x_main = -100.0
        #signal_x_main = signal_x_main * (1.0/200.0) + 0.5

        if signal_y_main > 100.0:
            signal_y_main = 100.0
        elif signal_y_main < -100.0:
            signal_y_main = -100.0
        #signal_y_main = signal_y_main * (1.0/200.0) + 0.5
